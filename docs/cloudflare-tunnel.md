...
### Самый простой способ — одна команда `npm run play`

Поднимает сервер и туннель разом и печатает готовую ссылку (сервер, который уже
работает, не трогает — вешает туннель на него):
...
`Ctrl+C` останавливает обоих. Остались процессы после закрытия окна крестиком —
`npm run tunnel:stop`. Подробности — в [hosting.md](hosting.md).

### Только туннель — скрипт `npm run tunnel:cf`
