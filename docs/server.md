...
| `core/model.js` | Фабрика `createRoomObject`, типы (JSDoc), `addLog`, `teamName`, `teamCounts`. Комната хранит голосование агентов (task 1): `votes` (`{cards, skip}`), `pendingVote` (идущий отсчёт) и внутренний `_voteTimeout` (не сериализуется). Плюс `marks` (`{red:{}, blue:{}}`) — клики ОЖИДАЮЩЕЙ команды по картам (task 5): на каждую команду `{индекс -> [id агентов]}`, выбор эксклюзивен. |

## `src/services/` — доменная логика

| Модуль | Содержимое |
|---|---|
| `services/gameEngine.js` | Партия: `startGame`, `giveClue`, `voteCard`, `voteSkip`, `handleVoterGone`, `markCard`, `removePlayerMarks`, `endTurn`, `checkWin`, `finishGame`, `pauseGame`, `resumeGame`, `returnToLobby`. Чистый — рассылку получает через `ctx.broadcast`. `giveClue` пишет подсказку в `room.clueHistory`. **Время хода лидера (task 3):** удлинённое `firstMoveTime` (120с) даётся ТОЛЬКО на самый первый ход партии — его выставляет `startGame`; все последующие ходы лидера через `endTurn` получают `answerTime` (60с) и никогда не превышают первый. Фаза угадывания (`giveClue`) — тоже `answerTime`. **Голосование агентов (task 1):** `voteCard`/`voteSkip` переключают голос игрока (эксклюзивно), пересчитывают единогласие и при согласии всех подключённых агентов команды запускают 1.5-сек отсчёт (`VOTE_COUNTDOWN_MS`) через `timerService.startCountdown`; по завершении внутренний `applyVote` открывает карту (`revealCard`) или передаёт ход (`endTurn`). Верная карта ход НЕ передаёт. `handleVoterGone` снимает голос ушедшего и пересчитывает согласие. **Клики ожидающей команды (task 5):** `markCard` эксклюзивно переключает клик агента команды, чьего хода НЕТ (карту не открывает, состояние не меняет); внутренние `resetMarks` (вызывается при `startGame`/`endTurn`/`finishGame`/`returnToLobby`) и `clearCardMarks` (в `revealCard`) чистят клики при смене хода и вскрытии карты; `removePlayerMarks` снимает клики ушедшего. `returnToLobby` (кнопка «Завершить игру» в настройках) достижима в т. ч. из паузы, поэтому снимает флаг `paused`. |
...
| `services/serializer.js` | `stateFor` — снимок состояния для конкретного игрока; единственная точка, где скрываются цвета карт. Отдаёт также `votes`/`pendingVote` (task 1) — секретов в них нет. Поле `marks` (task 5), наоборот, секрет: отдаются клики ТОЛЬКО команды игрока (`room.marks[me.team]`), сопернику/наблюдателю — `{}`. |
